...
**The one idea:** Gemini answers *what is physically in this drawing* (1). Code
decides *what it means to you* (2). Because the model never sees your weights,
perception caches — so a re-rank is 23ms with no model call, and swapping Vastu
for Feng Shui re-ranks everything without touching Gemini.

**The one idea:** Gemini answers *what is physically in this drawing*. Code decides
*what it means to you*. The model never sees your weights and never emits a score —
which is why perception caches, why a re-rank is 23ms with no model call, and why
swapping Vastu for Feng Shui re-ranks everything without touching the model.
